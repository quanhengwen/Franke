...
volatile uint8_t Down_finish = 4;
...
//float midwei = 0.0;
//float largewei = 0.0;
static uint16_t delaytt = 100;
static uint16_t delaytt1 = 1600;//45%
...
static uint8_t Downjiaozhun = 4;
static uint8_t fanqidong = 0;
static uint16_t chengzADC = 3913;
...
//									HAL_Delay(lifttime);
					...
					sprintf(Errorexplain,"Fill Time Out");
					Lcm_GotoXY(0,0,1);
...
void Switching_Value(void)
{
	...
	static uint8_t nodoor = 0;
	static uint8_t outdoor = 0;
	static uint8_t c_sensor = 0;
	static uint8_t v_sensor = 0;
	static uint8_t ssada = 0;
	/*if(HAL_GPIO_ReadPin(Outside_DoorOpen_GPIO_Port,Outside_DoorOpen_Pin)){
		static TIME outside;
...
//							HAL_Delay(3000);
					...
					if(custdsp.Line == 1 && stm32_config.CUSTSelected2 >= 1){
						uint8_t bx = 1;
						while(bx){
