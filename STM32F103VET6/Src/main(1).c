...
//	DeviceCheck();
	...
	Lcm_GotoXY(0,0,0);
	Lcm_Wr_Command(0x01); //显示清屏
	sprintf(asd,"重启步骤，刚开机不用理");
	Lcm_GotoXY(0,0,1);
	Lcm_Disp_String((unsigned char*)asd);
	Lcm_GotoXY(0,1,1);
	Lcm_Disp_String((unsigned char*)asd);
	Lcm_GotoXY(0,0,0);
	Lcm_Disp_String((unsigned char*)asd);
	...
	while(1){
		...
		Lcm_GotoXY(0,0,0);
		Lcm_Wr_Command(0x01); //显示清屏
		sprintf(asd,"升降电机测试， 不转??");
		Lcm_GotoXY(0,0,1);
		Lcm_Disp_String((unsigned char*)asd);
		sprintf(asd,"目前重量:%.0f",Measuing);
		Lcm_GotoXY(0,1,1);
		Lcm_Disp_String((unsigned char*)asd);
		sprintf(asd,"目标重量:%d",liftweight);
		Lcm_GotoXY(0,0,0);
		Lcm_Disp_String((unsigned char*)asd);
		Lcm_GotoXY(0,1,0);
		Lcm_Disp_String((unsigned char*)asd);
