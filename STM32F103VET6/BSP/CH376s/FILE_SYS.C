...
#include	"FILE_SYS.H"
//#include	"HAL.H"						/* 使用 USRT_HW.C 中封装的的串口通讯函数 */
#include "usart.h"
#include "host.h"
#include "LCD19264.h"
...
#else
	UINT32	i;
//	for ( i = 0; i < 12000000; i ++ ) {  /* 计数防止超时,默认的超时时间,与单片机主频有关 */
...
UINT8	CH376SendCmdWaitInt( UINT8 mCmd )  /* 发出命令码后,等待中断 */
{
	...
	xWriteCH376Cmd( mCmd );
	//xEndCH376Cmd( );
	//printf("UINT8	CH376SendCmdWaitInt( UINT8 mCmd ) \r\n");
